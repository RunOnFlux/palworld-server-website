...
const SEO = ({
  ...
  // Per-page Open Graph card, generated by scripts/gen-og-images.mjs from the page headline.
  // Derived from the same url prop the canonical uses, so the tag React hoists after
  // hydration matches the one scripts/prerender.mjs stamped into the served HTML.
  const ogSlug = (url || '/').replace(/^\//, '').replace(/\/$/, '').replace(/\//g, '-') || 'home';
  const seoImage = image
    ? `${siteUrl}${image}`
    : `${siteUrl}/games/palworld/og/${ogSlug}.webp`;
